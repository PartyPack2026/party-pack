...
class Bluff {
  ...
  nextRound() {
    ...
    this.io.to(this.code).emit('bluff_round', {
  ...
  startVoting() {
    ...
    this.io.to(this.code).emit('bluff_vote', {
  ...
  showResults() {
    ...
    this.io.to(this.code).emit('bluff_results', {
    ...
    Object.keys(this.room.players).forEach(id => {
      this.io.to(id).emit('bluff_results_player', {
  ...
  showFinalResults() {
    ...
    Object.keys(this.room.players).forEach(id => {
      this.io.to(id).emit('final_scores', { scores, gameName: 'Bluff' });
    });
