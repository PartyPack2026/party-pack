...
class Punchline {
  ...
  nextRound() {
    ...
    this.io.to(this.code).emit('punchline_round', {
  ...
  showNextVote() {
    ...
    this.io.to(this.code).emit('punchline_vote', {
      prompt: a1.prompt,
  ...
  showFinalResults() {
    ...
    Object.keys(this.room.players).forEach(id => {
      this.io.to(id).emit('final_scores', { scores, gameName: 'Punchline' });
    });
